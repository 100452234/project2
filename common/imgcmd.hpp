...
namespace images::common {
  ...
  void process_file(const std::filesystem::path & in_file, const std::filesystem::path & out_dir,
    ...
    try {
      ...
      auto [read_time, process_time, write_time] = generate_output<image_type>(out_dir, in_file,
          subcmd);
      std::array times = {write_time - start_time, read_time - start_time, process_time - read_time,

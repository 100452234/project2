...
namespace {
  ...
  static const std::map<std::string_view, subcommand> subcommand_map{
      {"copy",  subcommand::copy},
      {"histo", subcommand::histo},
      {"mono",  subcommand::mono},
      {"gauss", subcommand::gauss},
  ...
  void print_format_help(std::ostream & os, std::string_view prog_name) noexcept {
    ...
    os << "  " << prog.filename() << " in_path out_path oper\n";

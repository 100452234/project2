...
namespace images::soa {
  ...
  bitmap_soa::bitmap_soa(int w, int h) : header{w, h}, pixels{std::vector<uint8_t>(w * h),
                                                              std::vector<uint8_t>(w * h),
                                                              std::vector<uint8_t>(w * h)} {
  }

  void bitmap_soa::read(const std::filesystem::path & file_path) {
  ...
  void bitmap_soa::write(const std::filesystem::path & new_file) {
  ...
  void bitmap_soa::gauss() noexcept {
    ...
    for (int pixel_index = 0; pixel_index < num_pixels; ++pixel_index) {
      ...
      std::array<long, num_channels> value{};
      for (int gauss_index = 0; gauss_index < gauss_size; ++gauss_index) {
        ...
        value[red_channel] += gauss_value * pixels[red_channel][gauss_pixel_index];
        value[green_channel] += gauss_value * pixels[green_channel][gauss_pixel_index];
        value[blue_channel] += gauss_value * pixels[blue_channel][gauss_pixel_index];
      }
      result.pixels[red_channel][pixel_index] = value[red_channel] / gauss_norm;
      result.pixels[green_channel][pixel_index] = value[green_channel] / gauss_norm;
      result.pixels[blue_channel][pixel_index] = value[blue_channel] / gauss_norm;
    }
